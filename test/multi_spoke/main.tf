...
resource "aws_eip_association" "fmc_mgmt_ip_association" {
  count                = var.instances
  network_interface_id = aws_network_interface.mgmt[count.index].id
...
resource "aws_internet_gateway" "spkok1_int_gw" {
  vpc_id = aws_vpc.spoke1_vpc.id
  tags = {
    Name = var.spoke1_igw_name
  }
}
...
resource "aws_internet_gateway" "spkok2_int_gw" {
  vpc_id = aws_vpc.spoke2_vpc.id
  tags = {
    Name = var.spoke2_igw_name
  }
}

resource "aws_subnet" "spoke1" {
  count = length(local.spoke_az_distinct)

  vpc_id            = aws_vpc.spoke1_vpc.id
  availability_zone = local.spoke_az_distinct[count.index]
  cidr_block        = var.spoke1_subnet_cidr[count.index]
  tags = {
    Name = var.spoke1_subnet_name[count.index]
  }
}
...
resource "aws_subnet" "spoke2" {
  ...
  vpc_id            = aws_vpc.spoke2_vpc.id
  availability_zone = local.spoke_az_distinct[count.index]
  cidr_block        = var.spoke2_subnet_cidr[count.index]
  tags = {
    Name = var.spoke2_subnet_name[count.index]
  }
}