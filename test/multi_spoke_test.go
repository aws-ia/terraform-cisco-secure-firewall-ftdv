...
func TestMultiSpoke(t *testing.T) {
	...
	test_structure.RunTestStage(t, "fmc_config_dai", func() {
		...
		if err != nil {
			...
			fmt.Println("multi_spoke_test_with_fmc completed successfully. Output:", output)
		}
	})
	...
	defer test_structure.RunTestStage(t, "destroy_dai", func() {
		...
		time.Sleep(10 * time.Minute) // Wait before destroying
